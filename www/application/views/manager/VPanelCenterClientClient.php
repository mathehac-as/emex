...
<?php if(isset($client) && count($client) > 0): ?>
    ...
    <div class="modal-body">
        ...
        <input class="form-control input-sm-min" id="client_fio" value="<?=$client['fio']?>"
               placeholder="ФИО"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_emex_id">Emex ID</label>
        <input class="form-control input-sm-min" id="client_emex_id" value="<?=$client['emex_id']?>" 
               placeholder="Emex ID"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_vin">VIN</label>
        <input class="form-control input-sm-min" id="client_vin" value="<?=$client['vin']?>" 
               placeholder="VIN"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_marka_avto">Марка авто</label>
        <input class="form-control input-sm-min" id="client_marka_avto" value="<?=$client['marka_avto']?>" 
               placeholder="Марка авто"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_organization">Организация</label>
        <input class="form-control input-sm-min" id="client_organization" value="<?=$client['organization']?>" 
               placeholder="Организация"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_phone">Контактный телефон</label>
        <input class="form-control input-sm-min" id="client_phone" value="<?=$client['phone']?>" 
               placeholder="Контактный телефон"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_email">e-mail</label>
        <input class="form-control input-sm-min" id="client_email" value="<?=$client['email']?>" 
               placeholder="e-mail"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
        <label for="client_percent_discount">Процент скидки</label>
        <input class="form-control input-sm-min" id="client_percent_discount" value="<?=$client['percent_discount']?>" 
               placeholder="Процент скидки"<?=($act == 'edit' && !in_array(10, $accesses) ? ' readonly' : '')?>>
