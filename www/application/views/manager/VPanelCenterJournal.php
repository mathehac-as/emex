...
<?php if(isset($sum)):?>
    <table class="table table-condensed historys_sum">
        <tbody>
            <tr>
                <td><span>Оплачено наличными: <b><?=(float)$sum['journal_introduction_cash']?></b> руб.</span></td>
                <td><span>Пополнение баланса(нал): <b><?=(float)$sum['journal_coins_add']?></b> руб.</span></td>
                <td><span>Выдано из кассы: <b><?=(float)$sum['journal_return_cash']?></b> руб.</span></td>
            </tr>
            <tr>
                <td><span>Оплачено картой: <b><?=(float)$sum['journal_pay_card']?></b> руб.</span></td>
                <td><span>Пополнение баланса(безнал): <b><?=(float)$sum['journal_coins_add_card']?></b> руб.</span></td>
            </tr>
            <tr>
                <td><span>Перевод на карту: <b><?=(float)$sum['journal_transfer_card']?></b> руб.</span></td>
            </tr>
        </tbody>
    </table>
<?php endif; ?>
            ...
            <tbody>
                <?php if (isset($journals['office_historys'])): ?>
                    <?php foreach ($journals['office_historys'] as $value): ?>
                        <?php if($value['type_history'] == 'outcash' && isset($accesses) && in_array(4, $accesses)): ?>
                        ...
                        <?php elseif($value['type_history'] == 'incash' && isset($accesses) && in_array(5, $accesses)): ?>
